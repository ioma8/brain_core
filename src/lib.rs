...
impl MindMap {
    pub fn new() -> Self {
        ...
        Self { nodes, root_id: root_id.clone(), selected_node_id: root_id }
    }
                 ...
                 if let Some(node) = self.nodes.get(&current_id) {
                     if let Some(parent_id) = &node.parent {
                         if let Some(parent) = self.nodes.get(parent_id) {
                              if let Some(pos) = parent.children.iter().position(|id| id == &current_id) {
...
mod tests {
    ...
    fn test_add_child() {
        ...
        let child_id = map.add_child(&root_id, "Child 1".to_string()).expect("Failed to add child");
    ...
    fn test_change_node() {
        ...
        map.change_node(&root_id, "New Root Content".to_string()).expect("Failed to change node");
    ...
    fn test_add_sibling() {
        ...
        let sibling_id = map.add_sibling(&child1_id, "Sibling 1".to_string()).expect("Failed to add sibling");
        ...
        assert!(map.add_sibling(&root_id, "Root Sibling".to_string()).is_err());
    }
